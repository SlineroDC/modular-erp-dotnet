        ...
        var document = Document.Create(container =>
        {
            container.Page(page =>
            {
                ...
                page.Header().Row(row =>
                {
                    ...
                    {
                        col.Item().Text("ERP System").SemiBold().FontSize(24).FontColor(Colors.Blue.Darken3);
                ...
                page.Content().PaddingVertical(1, Unit.Centimetre).Column(col =>
                {
                    ...
                    col.Item().Table(table =>
                    {
                        ...
                        foreach (var detail in sale.SalesDetails)
                        {
                            ...
                            table.Cell().Element(ItemStyle).Text(detail.Product?.Name ?? "Unknown Item");
                            table.Cell().Element(ItemStyle).AlignRight().Text($"{detail.UnitPrice:C}");
                            table.Cell().Element(ItemStyle).AlignCenter().Text($"{detail.Quantity}");
                            table.Cell().Element(ItemStyle).AlignRight().Text($"{totalLine:C}");
                    ...
                    col.Item().AlignRight().Row(row => 
                    {
                        ...
                        row.ConstantItem(200).BorderTop(1).BorderColor(Colors.Black).PaddingTop(5).Row(totalRow => 
                        {
                            totalRow.RelativeItem().Text("GRAND TOTAL:").SemiBold().FontSize(14);
                            totalRow.RelativeItem().AlignRight().Text($"{sale.Total:C}").SemiBold().FontSize(14).FontColor(Colors.Green.Darken2);
                        });
                    });
                });
